    ...
    public class FileController : Controller
    {
        ...
        public async Task<IActionResult> Index(int id)
        {
            if (User.IsInRole("Teacher"))
            {
                ...
                var pfiles = await _context.PFiles.Where(u => u.LectureId == id && u.Lecture.Course.TeacherId == teacherId).ToListAsync();
        ...
        public async Task<IActionResult> Edit(int id)
        {
            if (User.IsInRole("Teacher"))
            {
                var video = await _context.PFiles.FindAsync(id);
        ...
        public async Task<IActionResult> Edit(int id, EditVideoVM courseForm)
        {
            if (User.IsInRole("Teacher"))
            {
                try
                {
                    ...
                    string oldImageFileName = video.FilePath;
                    string Oldd = video.FilePath;
                    ...
                    if (courseForm.VideoPath != null)
                    {
                        ...
                        {
                            var oldFilePath = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot", "files", video.FilePath);
        ...
        public async Task<IActionResult> Delete(int id)
        {
            ...
            {
                var pfile = await _context.PFiles.FindAsync(id);
                if (pfile == null)
                {
                    return NotFound();
                }
                ...
                var videoPath = Path.Combine("wwwroot", "files", pfile.FilePath);
                ...
                _context.PFiles.Remove(pfile);
                await _context.SaveChangesAsync();
                HttpContext.Session.SetString("deleted", "true");
                return RedirectToAction("Index", new { id = pfile.LectureId });
            }
            else
            {
                return RedirectToAction("Index", "Home");
